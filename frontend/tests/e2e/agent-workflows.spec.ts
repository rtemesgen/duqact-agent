...
test.describe.serial('agent workflows', () => {
  test('creates accounts, wallets, transactions, profiles, settings, and exchange-rate data', async ({ page }) => {
    ...
    await expect(page.getByRole('cell', { name: 'QA Channel Type' })).toBeVisible();
    await page.getByRole('button', { name: /service channels/i }).click();
    await page.getByRole('button', { name: /add service channel/i }).click();
    await page.getByLabel('Channel Type').selectOption({ label: 'QA Channel Type' });
    ...
    await expect(page.getByRole('cell', { name: 'QA Service Channel' })).toBeVisible();

    await openNav(page, 'Mobi Account Setting');
    await page.getByRole('button', { name: /add account/i }).click();
    await page.getByLabel('Service Channel').selectOption({ label: 'QA Service Channel (QA Channel Type)' });
    ...
    await expect(page.getByText('QA Account 1').first()).toBeVisible();
    ...
    await expect(page.getByText('QA Wallet 1').first()).toBeVisible();
    await page.reload();
    await openNav(page, 'Wallets');
    await expect(page.getByText('QA Wallet 1').first()).toBeVisible();
    ...
    await page.getByLabel('Client ID').fill('Desk Client');
    ...
    await expect(page.getByText('+256700123123').first()).toBeVisible();
    await page.getByRole('button', { name: /record transaction/i }).click();
    ...
    await page.getByLabel('Client ID').fill('History Client');
    await page.locator('.modalShade form').getByRole('button', { name: /^save$/i }).dblclick();
    await expect(page.getByText('+256700200200').first()).toBeVisible();
    await expect(page.locator('.modalShade')).toHaveCount(0);
    ...
    await expect(page.getByText(/QA Land/i).first()).toBeVisible();
    ...
    await expect(page.getByText('Mobi Agent QA').first()).toBeVisible();
    await page.reload();
    await expect(page.getByText('Mobi Agent QA').first()).toBeVisible();
