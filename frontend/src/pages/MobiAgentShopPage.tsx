...
export function MobiAgentShopPage() {
  ...
  return (
    <section className="pageSection">
      ...
      {view && <Modal title="Shop Details" onClose={() => setView(null)} onSubmit={(e) => { e.preventDefault(); setView(null); }} submitLabel="Close" size="lg"><div className="detailGrid"><div className="detailCard"><span>Business Name</span><strong>{view.businessName}</strong></div><div className="detailCard"><span>Agent ID</span><strong>{view.agentId}</strong></div><div className="detailCard"><span>Location</span><strong>{view.location}, {view.country}</strong></div><div className="detailCard"><span>Owner</span><strong>{view.ownerName}</strong></div><div className="detailCard detailCard-wide"><span>Remarks</span><strong>{view.remarks || '--'}</strong></div><div className="detailCard detailCard-wide"><span>Workers</span><strong>{(workers[view.id!] ?? []).length ? (workers[view.id!] ?? []).map((worker) => `${worker.userName} (${worker.jobTitle})`).join(', ') : 'No workers assigned.'}</strong></div>{session?.role === 'ADMIN' && view.id && (workers[view.id!] ?? []).length > 0 && <div className="detailCard detailCard-wide"><span>Manage Workers</span><div className="actionRow">{(workers[view.id!] ?? []).map((worker) => <button key={worker.id} className="iconButton dangerIcon" onClick={() => removeWorker(view.id!, worker.id!)} title={`Remove ${worker.userName}`}><UserX size={16} /></button>)}</div></div>}</div></Modal>}
    </section>
  );
}
