...
export function ApiDocumentationPage() {
  ...
  return (
    <section className="pageSection">
      <div className="pageHero"><div><p className="eyebrow">Microservice</p><h1>API Documentation</h1><p className="pageLead">Manage API and integration connection records.</p></div></div>
      ...
      {view && <Modal title="API Details" onClose={() => setView(null)} onSubmit={(e) => { e.preventDefault(); setView(null); }} submitLabel="Close"><div className="detailGrid"><div className="detailCard"><span>Name</span><strong>{view.name}</strong></div><div className="detailCard"><span>Status</span><strong>{view.status}</strong></div><div className="detailCard detailCard-wide"><span>Endpoint</span><strong>{view.endpoint}</strong></div><div className="detailCard detailCard-wide"><span>Description</span><strong>{view.description}</strong></div></div></Modal>}
