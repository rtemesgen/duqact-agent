...
export function ProfilePage() {
  ...
  return (
    <section className="pageSection profilePage">
      ...
      <div className="profileHeaderCard"><div className="profileIdentity"><div className="profileAvatar">{initials}</div><div><h1>{profile.name}</h1><p>{session?.role === 'ADMIN' ? 'Admin' : 'Mobi Agent'}</p><div className="profileMetaPill"><Mail size={14} />{profile.email}</div></div></div><div className="profileHeaderActions">{editing ? <><button type="button" className="secondaryButton" onClick={() => setEditing(false)}>Cancel</button><button type="button" className="primaryButton" onClick={save}>Save Changes</button></> : <button type="button" className="secondaryButton" onClick={() => setEditing(true)}><Edit3 size={16} />Edit Profile</button>}</div></div>
      <div className="profileGrid">
        ...
        <section className="surfaceCard"><div className="surfaceHead"><h2><ShieldCheck size={18} />Identity Verification</h2><span className="statusPill statusPositive"><CheckCircle2 size={14} />Verified</span></div><div className="formGrid formGrid-two"><label>ID Type<select value={profile.idType || ''} onChange={(e) => setProfile({ ...profile, idType: e.target.value })} disabled={!editing}><option>National ID</option><option>Passport</option><option>Driver License</option></select></label><label>ID Number<input value={profile.idNumber || ''} onChange={(e) => setProfile({ ...profile, idNumber: e.target.value })} disabled={!editing} /></label></div><div className="verificationMediaGrid"><div className="verificationSlot"><Camera size={18} /><span>ID Front</span><small>{profile.idFrontUrl ? 'Uploaded' : 'Not uploaded'}</small></div><div className="verificationSlot"><Camera size={18} /><span>ID Back</span><small>{profile.idBackUrl ? 'Uploaded' : 'Not uploaded'}</small></div><div className="verificationSlot"><Camera size={18} /><span>Selfie</span><small>{profile.selfieUrl ? 'Verified' : 'Not uploaded'}</small></div></div></section>
      </div>
    </section>
  );
}
